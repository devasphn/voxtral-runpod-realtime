...
class UnderstandingAudioProcessor:
    ...
    def __init__(
        ...
        self.energy_threshold = 100.0  # Lowered for better detection
        self.zcr_min = 0.001  # Lowered minimum zero crossing rate
        self.zcr_max = 0.5    # Raised maximum zero crossing rate
    ...
    async def process_audio_understanding(self, audio_data: bytes, websocket=None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # COMPLETELY FIXED: Perfect speech detection with aggressive settings
    ...
    def _detect_speech_fixed(self, pcm_data: bytes) -> bool:
        ...
        try:
            ...
            if self.vad_enabled and self.vad:
                try:
                    ...
                    if total_frames > 0:
                        ...
                        is_speech = speech_ratio > 0.15  # Lowered for better detection with mistral-common
            ...
            has_energy = rms_energy > self.energy_threshold  # 100.0 (lowered)
            has_speech_zcr = self.zcr_min <= zcr_normalized <= self.zcr_max  # 0.001-0.5 (widened)
            ...
            has_speech_spectrum = 50 <= spectral_centroid <= 8000  # Typical speech range (lowered)
    ...
    async def _process_complete_speech_segment_mistral_common(self, conn_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if speech_quality > 0.15:  # Lowered threshold for mistral-common
            ...
            # CRITICAL: Return PCM data directly for mistral-common processing
            return {
                "speech_complete": True,
                "audio_data": complete_audio,  # Raw PCM data
    ...
    def _analyze_speech_quality(self, pcm_data: bytes) -> float:
        ...
        try:
            ...
            # Energy analysis
            rms_energy = np.sqrt(np.mean(audio_array.astype(np.float64) ** 2))
            energy_score = min(1.0, rms_energy / 500.0)  # Lowered threshold for mistral-common
            
            # Dynamic range analysis
            max_amplitude = np.max(np.abs(audio_array))
            dynamic_score = min(1.0, max_amplitude / 5000.0) if max_amplitude > 0 else 0.0  # Lowered for mistral-common
            
            # Zero crossing rate analysis
            zero_crossings = np.sum(np.diff(np.signbit(audio_array)))
            zcr_normalized = zero_crossings / max(len(audio_array) - 1, 1)
            zcr_score = 1.0 if 0.001 <= zcr_normalized <= 0.5 else 0.6  # More forgiving for mistral-common
            ...
            return 0.2  # Default moderate quality for mistral-common
